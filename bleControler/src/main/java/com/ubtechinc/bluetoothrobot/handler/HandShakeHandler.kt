...
class HandShakeHandler : IMsgHandler {
    override fun handleMsg(requestCmdId: Short,  responseCmdId: Short, request: MiniMessage, peer: Object) {
        ...
        val responseBuilder = CodeMaoHandShake.HandShakeResponse.newBuilder()
        if(BleStatusManager.instance.isCodeMaoConnectionStart()) {
            LogUtils.w("isCodeMaoConnectionStart refuse!")
            responseBuilder.setIsSuccess(false).build()
            responseBuilder.resultCode = ERROR_ROBOT_BUSY
            ...
            SkillManager.instance.startCodeMaoConnectionSkill(object : ResponseCallback {
                override fun onFailure(p0: Request?, e: CallException?) {
                    responseBuilder.setIsSuccess(false).build()
                    responseBuilder.resultCode = fordReasonHandler(e!!)
                    LogUtils.w("start fail code = " + responseBuilder.resultCode  )
                    Robot2PhoneMsgMgr.get().sendResponseData(responseCmdId, Statics.PROTO_VERSION, requestSerial,
                            responseBuilder.build(), peer)
