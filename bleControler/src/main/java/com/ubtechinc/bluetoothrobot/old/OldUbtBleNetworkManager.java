...
public class OldUbtBleNetworkManager {
    private static final String TAG = OldUbtBleNetworkManager.class.getSimpleName();
    ...
    private com.ubtechinc.bluetoothrobot.BleNetworkWifiManager mNetworkWifiManager;
...
//                    break;
    ...
    private static class UbtDealBleCommandManagerHolder {
        public static OldUbtBleNetworkManager instance = new OldUbtBleNetworkManager();
    }

    private OldUbtBleNetworkManager() {
    }

    public static OldUbtBleNetworkManager getInstance() {
    ...
    public void init(Context context, String serialNumber) {
        ...
        commandEncode = JsonCommandEncode.get().init(serialNumber);
    ...
    void robotConnectToOtherDeviecs(final Object peer){
        try {
            ...
            Robot2PhoneOldMsg.Companion.getInstance().sendData(commandEncode.encryption(reply.toString()) , peer);
    ...
    private void sendWifiResultToMobile() {
        ...
        Robot2PhoneOldMsg.Companion.getInstance().sendData(commandEncode.encryption(commandProduce.getWifiSuc(mSerialNumber, mProductId)) ,  new BleSender("", 0));
    ...
    private void sendConnectSuccess(int code) {
        Robot2PhoneOldMsg.Companion.getInstance().sendData(commandEncode.encryption(commandProduce.getConnectSuc(code)) ,  new BleSender("", 0));
    }
    ...
    private void sendWifiIsOkMsgToPhone() {
        ...
        if ( LoginBussiness.getInstance(mContext).isLoginIM()) {//wifi可用
            Robot2PhoneOldMsg.Companion.getInstance().sendData(commandEncode.encryption(commandProduce.getWifiAvailable(mSerialNumber, mProductId)) ,  new BleSender("", 0));
        } else {
            Robot2PhoneOldMsg.Companion.getInstance().sendData(commandEncode.encryption(commandProduce.getWifiInvalid()) ,  new BleSender("", 0));
        }
    }
    ...
    private void sendWifiListToPhone(List<ScanResult> list) throws JSONException {
        ...
        Robot2PhoneOldMsg.Companion.getInstance().sendData(commandEncode.encryption(commandProduce.getWifiList(list)) ,  new BleSender("", 0));
    }
    ...
    public void sendErrorCodeToPhone(int errorCode) {
        Robot2PhoneOldMsg.Companion.getInstance().sendData(commandEncode.encryption(commandProduce.getErrorCode(errorCode)) ,  new BleSender("", 0));
    }
    /**
     * 向手机端发送绑定成功消息
     */
    public void sendMsgBingdingSuccessToPhone() throws JSONException {
        Robot2PhoneOldMsg.Companion.getInstance().sendData(commandEncode.encryption(commandProduce.getBindingSuc()), new BleSender("", 0));
...
//    }
    ...
    public void startScanWifi() {
        ...
        mScanWifiTimer.schedule(mScanWifiTask, TEN_SECONDS, TWO_SECONDS);
    }
