...
import com.ubtechinc.nets.im.business.ReceiveSocketMessageBussinesss;
...
public class RobotPhoneCommuniteProxy<T> {
    ...
    private IReceiveMsg receivePhoneMsgEngine = ReceiveMessageBussinesss.getInstance();
    private IReceiveMsg receivePcMsgEngine = ReceiveSocketMessageBussinesss.getInstance();
    ...
    public void init() {
        sendMsgEngine.init();
        receivePhoneMsgEngine.init();
        receivePcMsgEngine.init();
    }

    public void setIMsgDispatcher(ImMsgDispathcer msgDispathcer) {
        receivePhoneMsgEngine.setIMsgDispatcher(msgDispathcer);
        receivePcMsgEngine.setIMsgDispatcher(msgDispathcer);
    }

    /**
     * 发送回复消息
     */
    public void sendResponseMessage(final int cmdId, String version, long responseSerial, GeneratedMessageLite requestBody, String peer, final ICallback dataCallback) {
        sendData(cmdId, version, responseSerial, requestBody, peer, new Callback() {

            @Override
            public void onSendSuccess() {
                //消息发送成功，等待回调
                LogUtils.d(TAG, "onSendSuccess---cmdId : " + cmdId);
            }
