...
import com.ubtechinc.alpha.appmanager.model.UbtAppButtonEventData;
import com.ubtechinc.alpha.appmanager.model.UbtAppConfigData;
...
public class AppManager {
    ...
    private static final String CHAT_PKG = "";
    private static final String TRANSFER_PKG = "";
    ...
    public void startApp(Uri uri){
        ...
        if (!"ubtechinc".equals(scheme) || !"com.ubtechinc".equals(host))
            return;
...
//        registerReceiver();
    ...
    private void registerReceiver() {
        ...
        intentFilter2.addAction("appconfigback");
        intentFilter2.addAction("buttonback");
    ...
    public synchronized void stopApp(String packageName) {
    ...
    private void handleAppDied(String pkgName) {
        synchronized (this) {
            ...
            for (String packageName : stopingAppList) {
                LogUtils.d(TAG, "handleAppDied  stopingAppList = " + packageName);
            }

            if (stopingAppList.contains(pkgName)) {// 由主服务主动杀掉该app
                stopingAppList.remove(pkgName);
                LogUtils.d(TAG, "handleAppDied  initiative stop");
            } else { //应用意外退出
                if (pkgName.equals(FIRST_APP_DEFAULT) && !shutdown) {
                    LogUtils.d(TAG, "handleAppDied  unusual stop, retart default app , stopingAppList.size = "+ stopingAppList.size());
                    startDefaultApp();
                }
            }
        }
    ...
    public  class AppPackageBroadcast extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            }else if (action.equals("appconfigback")) {
