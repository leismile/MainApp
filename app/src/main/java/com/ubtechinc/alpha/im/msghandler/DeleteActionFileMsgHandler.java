...
public class DeleteActionFileMsgHandler implements IMsgHandler {
    ...
    public void handleMsg(int requestCmdId, int responseCmdId, AlphaMessageOuterClass.AlphaMessage request, String peer) {
        ...
        Cursor cursor = resolver.query(ActionStore.Action.getContentUri(),
                null, ACTION_ID + " = ? OR " +
                        ActionStore.Action.Columns.EN_NAME + " = ? OR " +
                        ActionStore.Action.Columns.CN_NAME + " = ? ",
                new String[]{deleteActionFile.getFileName(),
                        deleteActionFile.getFileName(),
                        deleteActionFile.getFileName()}, null);
        ...
        if (cursor != null && cursor.getCount() == 1) {
            ...
            actionID = cursor.getString(cursor.getColumnIndex(ACTION_ID));
            resolver.delete(ActionStore.Action.getContentUri(),
                    ACTION_ID + " = ?",
                    new String[]{actionID});
            // 删除其文件
